...
- users (id, name, email, password_hash, role, study_streak, last_study_date, created_at)
- classrooms (class_id [PK], class_name, facilitator_id [FK -> users.id ON DELETE CASCADE], class_code [Unique], created_at)
- enrollments (enrollment_id [PK], student_id [FK -> users.id], class_id [FK -> classrooms.class_id], joined_at, UNIQUE(student_id, class_id))
- documents (id, title, file_type, file_path, status, class_id [FK -> classrooms.class_id], uploader_id [FK -> users.id], created_at)
- summaries (id, document_id [FK -> documents.id], overview, sections_json, created_at)
- flashcards (id, student_id, document_id, question, answer, bloom_level, next_review, ease_factor, interval, repetitions)
- quizzes_classroom (id [PK], classroom_id [FK -> classrooms.class_id ON DELETE CASCADE], created_by [FK -> users.id], title, bloom_distribution, created_at)
- questions_classroom (id [PK], quiz_id [FK -> quizzes_classroom.id ON DELETE CASCADE], text, type, options_json, correct_answer, bloom_level, marks)
